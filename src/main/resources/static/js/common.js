...
let mouseX = -Infinity, mouseY = -Infinity; // 단축키 줌 기준점 계산용 마우스 위치
...
// 슬라이더 입력값(100~400)을 백분율로 해석해 배율 적용 (현재 보이는 화면 가운데 기준 유지)
function applyZoomPreset(pct) {
  const cx = pdfScroll.clientWidth / 2;
  const cy = pdfScroll.clientHeight / 2;
  const canvasX = (pdfScroll.scrollLeft + cx) / scale;
  const canvasY = (pdfScroll.scrollTop  + cy) / scale;
  scale = Math.max(fitScale * minZoomMult, Math.min(5, pct / 100));
  applyTransform();
  pdfScroll.scrollLeft = canvasX * scale - cx;
  pdfScroll.scrollTop  = canvasY * scale - cy;
  scheduleRerender();
}
...
document.addEventListener('keydown', e => {
  ...
  if (e.key >= '1' && e.key <= '5') {
    ...
    if (inside) {
      zoomAt(mouseX, mouseY, +e.key / scale);
    } else {
